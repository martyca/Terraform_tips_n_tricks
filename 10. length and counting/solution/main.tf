...
resource "null_resource" "second_batch" {
    count = 3
    triggers = {
        name = "second_batch ${length(null_resource.first_batch) + count.index}"
    }
}
...
resource "null_resource" "third_batch" {
    count = 3
    triggers = {
        name = "third_batch ${length(null_resource.first_batch) + length(null_resource.second_batch) + count.index}"
    }
}
